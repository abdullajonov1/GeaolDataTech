...
export function Footer({locale}:{locale:Locale}){
 ...
 <div className="footer-bottom"><span>© {new Date().getFullYear()} GEOLDATA TECH. {t.rights}</span><div><Link href={`/${locale}/privacy-policy`}>{t.privacy}</Link><Link href={`/${locale}/terms`}>{t.terms}</Link><a href="#top" aria-label={t.back}><ArrowUp/></a></div></div></footer>
}
